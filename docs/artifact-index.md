...
| React Liquid Glass Demo 与浏览器 QA | `frontend/`、`reports/figures/week8_react_demo_desktop.png`、`reports/figures/week8_react_demo_mobile.png`、`reports/week8_react_demo_qa.md` | 用户提供图片 SHA-256 `0364ff44229c70666216343057f9ae77d82438a7f842b30af1ffabb786061a7e` 是 no verified ground truth 的 out-of-domain 田间样例；ResNet-50/MPS top-1 `Cercospora leaf spot Gray leaf spot`、`0.870144` 仅为 prediction。可选 `mlx-community/Qwen3-VL-4B-Instruct-4bit` 因默认缓存无权重而 `ready=false`，UI unavailable state 已验证，no automatic download |
