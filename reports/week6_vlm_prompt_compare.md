...
| question_type | correct | total | exact_match |
| --- | ---: | ---: | ---: |
| condition | 1 | 5 | 0.2000 |
| health_status | 5 | 5 | 1.0000 |
| plant | 5 | 5 | 1.0000 |

Top confusions:

- condition: expected `Leaf Mold`, got `early blight` (4x)
...
- condition: expected `Leaf Mold`, got `northern leaf blight` (3x)
- condition: expected `Leaf Mold`, got `powdery mildew` (1x)
...
## Prompt Comparison Takeaways

- `short` fixed the answer-format problem for plant and health-status questions:
  both improved to `5/5`, while the original verbose prompt scored `0/5` on all
  three question types under strict exact match.
- `choice` and `few_shot_choice` improved total exact match from `10/15` to
  `11/15` and removed automatic risk-word flags.
- The condition task remained weak: `choice` and `few_shot_choice` reached only
  `1/5`. Closed choices reduced free-form hallucination, but did not solve
  fine-grained disease-condition recognition.
- Public materials should describe this as a resource-limited VLM smoke
  comparison, not LoRA training or field-ready diagnosis.
